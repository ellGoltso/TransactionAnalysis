...
def get_general_information(data: pd.DataFrame) -> list[dict]:
    ...
    sum_operations_by_card['Сумма операции с округлением'] = sum_operations_by_card['Сумма операции с округлением'].astype(float)
    ...
    for index, value in enumerate(card_numbers):
        info_dict = {'last_digits': value, 'total_spent': round(sums_list[index], 2),
...
def get_top_five_transactions(data: pd.DataFrame) -> list[dict]:
    ...
    slice_of_values['Дата операции'] = slice_of_values['Дата операции'].apply(lambda x: datetime.date(x.year, x.month, x.day))
